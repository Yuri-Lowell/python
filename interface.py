...
class CSharpToJavaConverter:
    ...
    def _convert_interface_line(self, line: str) -> str:
        ...
        # 处理泛型
        line = re.sub(r'<([^<>]+)>', lambda m: f'<{self.convert_type(m.group(1))}>', line)
    ...
    def _convert_method_line(self, line: str) -> str:
        ...
        indent, return_type, method_name, params, _ = method_match.groups() if len(method_match.groups()) >= 4 else (method_match.groups() + (None,))
        
        # 转换返回类型
        new_return_type = self.convert_type(return_type.strip())
        
        # 转换方法名为驼峰命名（首字母小写）
        new_method_name = self.to_camel_case(method_name)
        
        # 转换参数
        new_params = self._convert_parameters(params)
        
        # 构建新行
        new_line = f'{indent}{new_return_type} {new_method_name}({new_params});'
    ...
    def _convert_property_line(self, line: str) -> str:
        ...
        if match:
            ...
            setter = f'    void {setter_name}({java_type} {prop_name});'
            return f'{getter}\n    {setter}'
        ...
        if match:
            ...
            java_type = self.convert_type(prop_type)
            getter_name = self.to_camel_case(f'Get{prop_name}')
            return f'    {java_type} {getter_name}();'
    ...
    def _convert_event_line(self, line: str) -> str:
        ...
        if match:
            ...
            add_name = self.to_camel_case(f'Add{event_name}Listener')
            remove_name = self.to_camel_case(f'Remove{event_name}Listener')
            return (f'    void {add_name}({java_type} listener);\n'
                    f'    void {remove_name}({java_type} listener);')
        return line
    ...
    def _convert_types_in_line(self, line: str) -> str:
        ...
        # 转换Entity类型（去掉I前缀）
        # 匹配以I开头后跟大写字母的单词
        def replace_entity_type(match):
            type_name = match.group(1)
            return self.remove_i_prefix(type_name)
        
        line = re.sub(r'\b(I[A-Z][a-zA-Z0-9]*)\b', replace_entity_type, line)
        ...
        line = re.sub(r'Task<([^>]+)>', lambda m: f'CompletableFuture<{self.convert_type(m.group(1))}>', line)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='C#接口转Java接口工具 - 方法名首字母小写，自动删除I前缀',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
转换规则:
  1. 接口名: IUserRepository -> UserRepositoryService
  2. 方法名: GetUserById -> getUserById (首字母小写)
  3. Entity类型: IUser -> User, IOrder -> Order
  4. 返回类型: Task<IUser> -> CompletableFuture<User>
  5. 参数类型: void Save(IUser user) -> void save(User user)
  6. string类型: string -> String
